...
namespace utils
{
	namespace path
	{
		...
		{
			CHECK(basename("") == "");
			std::string pathname = (separator + std::string("usr")).append({separator}).append(std::string("bin")).append({separator}).append("vim");
		...
		{
			size_t pos = pathname.find_last_of(separator);
		...
		std::string fileextension(const std::string &pathname)
		{
			...
			size_t start = bname.find_first_not_of('.');
