...
public:
    ...
    void addPoints(const T1 *x, const T2 *y, size_t uElems)
    {
    ...
    {
        return !(m_data.empty() || x < m_data.front().first || x > m_data.back().first);
    }
    ...
    {
        if(m_data.empty() || x < m_data.front().first || x > m_data.back().first)
            return notFoundItem;
