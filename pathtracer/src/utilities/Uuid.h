...
public:
    ...
    explicit Uuid(const std::string& rcsUuid)
    {
        ...
        for (size_t i = 0; i < rcsUuid.length() && j < 32; i++) {
            ...
            if(c < 0)
                throw ValueError(std::string("Unexpected character '").append({character}).append("' in uuid"));
    ...
    Uuid& operator=(const Uuid& rUuid) noexcept {
        if (this != &rUuid) {
    ...
    Uuid& operator=(Uuid&& rUuid) noexcept {
        if (this != &rUuid) {
            m_u64UuidLo = rUuid.m_u64UuidLo;
            m_u64UuidHi = rUuid.m_u64UuidHi;
        }
...
namespace std
{
    ...
    struct hash<Uuid>
    {
        size_t operator()(const Uuid &rUuid) const
        {
            return static_cast<size_t>(rUuid.getLo());
        }
    };
}
/// @endcond
